...
**Le moteur répond parfois qu’il n’y a pas de chemin.** Vingt-huit tronçons du Tour des Alpes reçoivent un 404 : la gorge du Höllentalklamm, dont le sentier à péage n’est pas routable, et la rive est du Königssee, où l’on n’accède qu’en bateau. Ce n’est pas une panne à corriger : il n’existe pas de voie dans OpenStreetMap, et le segment droit est la réponse honnête. Le rapport les liste avec leurs coordonnées, pour qu’on puisse les rejouer à la main plutôt que de les supposer.

*Note d’exploitation, 31 août 2026.* `api.openrouteservice.org` est dépréciée depuis avril 2026 au profit de `api.heigit.org/openrouteservice/`. L’ancienne adresse a d’abord vu son quota raboté, puis a rendu un 403 sec, sans corps, alors que le tableau de bord affichait un quota intact : rien dans la réponse ne désignait l’adresse. La clé, elle, est la même sur les deux.
